import { MANUAL_FOLLOW_UP_ALERT_THRESHOLD, evaluatePracticeSchedule } from './practiceMetrics.js';
import { evaluateGameSchedule } from './gameMetrics.js';

const formatPercentage = (value) => (value * 100).toFixed(1).replace(/\.0$/, '');

export function filterRedundantCapacityWarnings(dataQualityWarnings = [], overbookedSlots = []) {
  ...
  return dataQualityWarnings.filter((warning) => {
    ...
    const referencesOverCapacity =
      normalizedWarning.includes('over capacity') || normalizedWarning.includes('exceeds capacity');
