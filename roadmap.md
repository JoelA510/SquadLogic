...
- [ ] Practice Scheduling – Scheduling workflow outlined in `docs/practice-scheduling.md`; initial slot allocator implemented in `src/practiceScheduling.js` with unit tests, swap-based conflict resolution for unique slot requirements, and metrics reporting now covered by `src/practiceMetrics.js` (including base-slot division distribution summaries for fairness tracking, dominance warnings surfaced to the evaluator, and new alerts when more than 5% of teams require manual follow-up), while the manual override tooling is exposed through the `lockedAssignments` parameter, the allocator now penalises stacking the same division on a shared base slot to improve field fairness, the new seasonal slot expander `src/practiceSlotExpansion.js` normalises early/late windows ahead of scheduler integration, and the admin shell now surfaces a practice readiness snapshot while Supabase-backed workflows remain outstanding.
