...
- [ ] Evaluation & Refinement – Evaluation pipeline, fairness metrics, and remediation workflow documented in `docs/evaluation.md`; automated schedule checks now include `src/practiceMetrics.js` for practices and the newly added `src/gameMetrics.js` for games, with Supabase orchestration and UI reporting outstanding.
