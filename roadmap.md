...
- [ ] Team Generation – Core allocator implemented in `src/teamGeneration.js` with deterministic unit tests; conflict detection and validation safeguards added and overflow tracking via the new `overflowByDivision` return channel surfaces manual follow-ups while UI wiring and admin workflows remain outstanding.
