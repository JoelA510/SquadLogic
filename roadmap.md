...
- [ ] Team Generation – Core allocator implemented in `src/teamGeneration.js` with deterministic unit tests; conflict detection and validation safeguards added, overflow tracking via the new `overflowByDivision` return channel surfaces manual follow-ups and the new `overflowSummaryByDivision` rollup highlights impacted players per reason for triage, buddy diagnostics now expose mutual pairs and unresolved requests for admin review through `buddyDiagnosticsByDivision`, coach coverage reporting via `coachCoverageByDivision` highlights divisions lacking enough volunteers, the latest roster balance telemetry returned through `rosterBalanceByDivision` pinpoints teams needing players to streamline roster review, `src/rosterSizing.js` standardizes default roster caps from division formats and now converts Supabase roster override rows into season-aware config maps, the new summary helper `src/teamDiagnostics.js` assembles dashboard-ready division metrics with per-reason player overflow tallies for dashboard triage, duplicate player identifiers are now rejected early to protect data quality, and the admin shell now showcases a representative division snapshot while Supabase-backed workflows remain outstanding.
 - [ ] Practice Scheduling – Scheduling workflow outlined in `docs/practice-scheduling.md`; initial slot allocator implemented in `src/practiceScheduling.js` with unit tests, swap-based conflict resolution for unique slot requirements, and metrics reporting now covered by `src/practiceMetrics.js` (including base-slot division distribution summaries for fairness tracking, dominance warnings surfaced to the evaluator, new day-concentration alerts when divisions are stacked on a single practice day, readiness alerts now centralized in the evaluation pipeline so the manual follow-up threshold fires once with detailed breakdowns, underutilized base slot alerts to highlight wasted capacity, per-division base-slot load snapshots for readiness dashboards, and the fresh manual follow-up reason summaries used by the evaluation pipeline now broken down by capacity, coach availability, and excluded-slot constraints with category callouts surfaced to the evaluator), while the manual override tooling is exposed through the `lockedAssignments` parameter, the allocator now penalises stacking the same division on a shared base slot to improve field fairness with season-configurable scoring weights and discourages concentrating divisions on a single practice day when alternatives exist, now exposes per-division base-slot and day load snapshots to feed readiness dashboards, the new seasonal slot expander `src/practiceSlotExpansion.js` normalises early/late windows ahead of scheduler integration, the evaluator now escalates over-capacity practice slots as blocking issues for readiness triage with duplicate warnings filtered for clarity, the admin shell now surfaces a practice readiness snapshot with base-slot distribution, day-coverage, underutilization insights, day-concentration alerts, and manual follow-up category rollups, and Supabase-backed workflows now include a normalizer for `practice_slots` rows plus a helper to expand those slots into season-phase aware effective windows with season overrides preserved during normalization, dedicated persistence scaffolding now builds `practice_assignments` rows from scheduler output, and the new `persistPracticeAssignments` helper writes those rows through a supplied Supabase client with a no-op path when no assignments are present while storage write paths remain outstanding.
- [ ] Game Scheduling – Game scheduling blueprint refined in `docs/game-scheduling.md` with clarified inputs and fairness metrics; round-robin generator and the slot allocator in `src/gameScheduling.js` now rotate divisions across shared fields to avoid hogging, favour consistent start times when prior assignments establish team preferences, emit shared-slot telemetry for downstream analysis, and are covered by regressions in `tests/gameScheduling.test.js`, while admin conflict resolution tooling remains outstanding; Supabase persistence is now scaffolded via `src/gameSupabase.js` for inserting scheduler assignments, unscheduled matchup alerts now surface error-level summaries with reason breakdowns to speed admin triage, division-level unscheduled breakdowns now accompany those alerts to focus follow-up on impacted divisions, and the admin shell now previews evaluator outputs with unscheduled matchup and shared slot alerts to guide follow-up.
...
- [ ] Front-End Development – Navigation, screen blueprint, and state management approach documented in `docs/frontend-architecture.md`; the initial Vite-powered admin shell now lives in `frontend/` as a roadmap dashboard prototype with expanded practice readiness insights (base-slot distribution, division day coverage, and underutilization alerts), while Supabase client wiring and production-ready component implementation remain pending.
