...
- [ ] Team Generation – Core allocator implemented in `src/teamGeneration.js` with deterministic unit tests; conflict detection and validation safeguards added, overflow tracking via the new `overflowByDivision` return channel surfaces manual follow-ups, buddy diagnostics now expose mutual pairs and unresolved requests for admin review through `buddyDiagnosticsByDivision`, coach coverage reporting via `coachCoverageByDivision` highlights divisions lacking enough volunteers, and the latest roster balance telemetry returned through `rosterBalanceByDivision` pinpoints teams needing players to streamline roster review, while `src/rosterSizing.js` standardizes default roster caps from division formats, the new summary helper `src/teamDiagnostics.js` assembles dashboard-ready division metrics, and UI wiring plus admin workflows remain outstanding.
- [ ] Practice Scheduling – Scheduling workflow outlined in `docs/practice-scheduling.md`; initial slot allocator implemented in `src/practiceScheduling.js` with unit tests, swap-based conflict resolution for unique slot requirements, and metrics reporting now covered by `src/practiceMetrics.js` (including base-slot division distribution summaries for fairness tracking and dominance warnings surfaced to the evaluator), while the manual override tooling is exposed through the `lockedAssignments` parameter, the allocator now penalises stacking the same division on a shared base slot to improve field fairness, the new seasonal slot expander `src/practiceSlotExpansion.js` normalises early/late windows ahead of scheduler integration, and admin UI remains to be built.
