...
- [ ] Team Generation – Core allocator implemented in `src/teamGeneration.js` with deterministic unit tests; conflict detection and validation safeguards added and overflow tracking via the new `overflowByDivision` return channel surfaces manual follow-ups, while `src/rosterSizing.js` now standardizes default roster caps from division formats and UI wiring plus admin workflows remain outstanding.
- [ ] Practice Scheduling – Scheduling workflow outlined in `docs/practice-scheduling.md`; initial slot allocator implemented in `src/practiceScheduling.js` with unit tests, swap-based conflict resolution for unique slot requirements, and metrics reporting now covered by `src/practiceMetrics.js`, while the manual override tooling is exposed through the `lockedAssignments` parameter and admin UI remains to be built.
