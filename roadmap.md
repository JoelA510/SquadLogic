...
- [ ] Data Modeling & Storage – Schema blueprint drafted in `docs/data-modeling.md`; ingestion pipeline specification captured in `docs/ingestion-pipeline.md`; initial DDL in `docs/sql/initial_schema.sql` now adds enum-backed coach preferences, timestamp triggers, double-booking guards, and run/evaluation/export tracking tables for observability while awaiting Supabase validation; sample Fall 2024 dataset scripted in `docs/sql/sample_seed_data.sql` now keeps coach, team, and game rows idempotent via complete upserts to accelerate local testing; RLS strategy drafted in `docs/rls-policies.md` with migrations still outstanding.
